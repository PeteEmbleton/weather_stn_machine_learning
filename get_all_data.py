    ...
    for i in range(len(all_stations['items'])):
        ...
        with urllib.request.urlopen(url) as url:
            ...
            for j in range(len(station_data['items'])):
                ...
                try:
                    time_to_convert = str(all_stations['items'][j]['reading_timestamp'])
                    reading_timestamp_out = str((parse(time_to_convert))).split('+', 1)[0]
                ...
                #print(stn_id, time_to_convert, reading_timestamp_out, "j = ",j)
                data_exists = (cursor.execute("""SELECT reading_timestamp FROM weather_stn WHERE weather_stn_id = '%s' and reading_timestamp = '%s'""" %(stn_id,reading_timestamp_out)))
                #print(data_exists)

                if data_exists == 0:
                    stn_data = (stn_id, ambient_temp, ground_temp, air_quality, air_pressure, humidity, wind_direction, wind_speed, wind_gust_speed, rainfall, reading_timestamp_out)
                       ...
                       continue_loop = 0
                        ...
                        with urllib.request.urlopen(next_url) as url:
                            ...
                            for k in range(len(station_data['items'])):
                                ...
                                #print(stn_id, time_to_convert, reading_timestamp_out, "k = ",k)
                                data_exists = (cursor.execute("""SELECT reading_timestamp FROM weather_stn WHERE weather_stn_id = '%s' and reading_timestamp = '%s'""" %(stn_id,reading_timestamp_out)))
                                #print(data_exists)
